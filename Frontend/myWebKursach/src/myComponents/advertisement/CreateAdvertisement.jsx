...
export default function CreateAdvertisementForm() {
  ...
  useEffect(() => {
    async function fetchFormData() {
      setLoading(true);
      try {
        const result = await getCreateAdvertisementForm();
        if (result.success) {
          setFormData(result.data);
          setDescription(result.data.description || "");
        //  setSelectedAmenities(result.data.createAdvertisementAmenities);
        ...
        <Box key={amenity.amenity}>
          ...
          {amenity.isActive && requiresInput(amenity) && (
            <Input
              mt={2}
              type="number"
              placeholder={`Введите значение для ${amenity.amenityDisplay}`}
